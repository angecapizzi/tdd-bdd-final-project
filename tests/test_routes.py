...
class TestProductRoutes(TestCase):
    ...
    def test_update_product_not_found(self):
        """It return a HTTP 404 Not found code on Update not existing Product"""
    ...
    def test_query_by_name(self):
        ...
        for read_product in data:
            self.assertEqual(read_product["name"], test_name)
    ...
    def test_query_by_category(self):
        ...
        logging.debug("Found product [%d] %s", found_count, found)
        response = self.client.get(BASE_URL, query_string=f"category={category.name}")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        data = response.get_json()
        self.assertEqual(len(data), found_count)
        for read_product in data:
            self.assertEqual(read_product["category"], category.name)
    ...
    def test_query_by_availability(self):
        ...
        logging.debug("Available products [%d] %s", count, available_products)
        response = self.client.get(BASE_URL, query_string="available=true")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        data = response.get_json()
        self.assertEqual(len(data), count)
        for read_product in data:
            self.assertEqual(read_product["available"], True)
